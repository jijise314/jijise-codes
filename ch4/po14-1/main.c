...
int main() {
	...
	do {
		...
		switch (menu) {
		case 0: printf("\n모든 메모리를 해제(destroy)하고 프로그램을 종료합니다.\n");  destroyLinkedList(myList);
			break;

		case 1: printf("\n맨 앞에 추가할 정수(value)를 입력하세요 : ");
			scanf("%d", &value);
			insertFirstLinkedList(myList, (PointType) { value, value, NULL, NULL });
			break;

		case 2: printf("\n맨 뒤에 추가할 정수를 입력하세요 : ");
			scanf("%d", &value);
			insertLastLinkedList(myList, (PointType) { value, value, NULL, NULL });
			break;
		...
		case 3: printf("\n추가할 위치와 정수를 입력하세요(순서-> 위치 정수) : ");
			...
			if (pos < 0 || pos > sizeLinkedList(myList)) {
				...
				insertAtLinkedList(myList, pos, (PointType) { value, value, NULL, NULL });
			}
			break;
